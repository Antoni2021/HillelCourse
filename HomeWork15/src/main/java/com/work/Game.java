...
public class Game {
    ...
    public static void start() {
        ...
        while (countOfGames-- != 0) {
            ...
            String resultOfGame = Hand.hand.get(handNum) + " vs " + Hand.hand.get(randomElement) +
                    ": " + GameService.chooseResults(result[handNum][randomElement]);
            System.out.println(resultOfGame);
            results.put(count++, resultOfGame);
        }
        System.out.println("You win " + wins + " time(s);\nYou loose " + loses +
                    " time(s);\nIt was " + draws + " draw(s).");
    }
}
